...
# # Replace the /api/price-trend endpoint in your app.py with this improved version
# # Replace the /api/price-trend endpoint in your app.py with this FIXED version

# @app.route("/api/price-trend", methods=["POST"])
# def get_price_trend():
#     """
#     FIXED: Get historical price trend for a specific car
#     NOW: Uses REAL market data when available, shows actual historical prices
#     """
#     try:
#         data = request.json
...
#         # ==========================================
#         # SCENARIO 3: STANDARD DEPRECIATION MODEL
#         # ==========================================
#         else:
#             print("⚠️ No matching data, using standard depreciation model")
            
#             # Standard depreciation curve (more realistic for Pakistani market)
#             depreciation_rates = {
#                 0: 0.00,   # Purchase year
#                 1: 0.15,   # 15% in first year
#                 2: 0.12,   # 12% in second year
#                 3: 0.10,   # 10% in third year
#                 4: 0.08,   # 8% in fourth year
#                 5: 0.08,   # 8% in fifth year
#             }
#             base_rate = 0.07  # 7% per year after 5 years
...
#             for i in range(car_age, -1, -1):
#                 year = registered_year + i
#                 age_at_that_time = car_age - i
                
#                 if age_at_that_time == 0:
#                     # Current year
#                     price = predicted_price
#                 else:
#                     # Calculate original price (reverse depreciation)
#                     rate = depreciation_rates.get(age_at_that_time, base_rate)
#                     price = current_value / (1 - rate)
#                     current_value = price
...
#             print(f"✅ Generated trend using standard depreciation")
...
#                     f"{(price_at_purchase/100000):.2f} Lacs in {registered_year}. "
#                     f"Current value: {(current_value/100000):.2f} Lacs. "
#                     f"{'Value increased due to market inflation.' if total_change > 0 else 'Normal depreciation observed.'}"
#                 )
#             }
#         })
